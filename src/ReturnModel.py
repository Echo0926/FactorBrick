...
class ReturnModel_Backtest:
    def __init__(self, session, pool, config,
        ...
        self.Quantile=config["Quantile"]    # 按因子大小分组数分层回测
    ...
    def summary_command(self):
        ...
        for (benchmark_str in ["{self.benchmark}"]){{    
            ...
            if (int({int(self.SingleFactor_estimation)})==1){{  // 说明需要进行单因子测试     
                // 1.2.Summary结果
                COUNTER=0;
                distinct_period_list=sort(exec distinct(period) from pt,true);
                for (p in distinct_period_list){{  
                    // Report
                    print("current_period: "+string(p)); 
                    
                    // Data
                    reg_df=select * from pt where period<=p and period>p-{self.callBackPeriod} and not isNull(R); 
    ...
    def Multisummary_command(self):
        ...
        for (benchmark_str in ["{self.benchmark}"]){{    
            ...
            // 格式调整!!
            final_pos_result=select benchmark_str as Benchmark,period,class,indicator,value from final_pos_result order by Benchmark,period;
