...
class ReturnModel_Data:
    ...
    def addLongDayFreqFactorFromDayFreqData(self: ReturnModel_Backtest):
        ...
        // 日频行情
        data = select ts_code as symbol,trade_date as date,1500 as minute,
        open*adj_factor as open,high*adj_factor as high,low*adj_factor as low,close*adj_factor as close,
        volume,amount as turnover from loadTable("dfs://DayKDB","o_tushare_a_stock_daily") 
        where (trade_date between temporalAdd(date({self.start_dot_date}),-1,"d") and date({self.end_dot_date})) and ts_code in code_list;        
        
        // 日频基本信息
        basic = select ts_code as symbol,trade_date as date,turnover_rate,total_mv,circ_mv from loadTable("dfs://DayKDB","o_tushare_a_stock_daily_basic") 
        where (trade_date between temporalAdd(date({self.start_dot_date}),-1,"d") and date({self.end_dot_date})) and ts_code in code_list;
        data = lj(data,basic,`symbol`date);
        data.append!(select * from data context by symbol limit -1) // 新增一个截面的数据
        
        // 隔夜收益率
        update data set interDayReturn = prev(nullFill((open-prev(close))\prev(close),0.0)) context by symbol;
        update data set interDayReturn_avg20 = mavg(interDayReturn,20) context by symbol; // 日间收益率均值 
        update data set interDayReturn_std20 = mstd(interDayReturn,20) context by symbol; // 日间收益率波动率
        
        // 隔夜换手率
        update data set turnoverRateDiff = prev(turnover_rate - prev(turnover_rate)) context by symbol; 
        update data set turnoverRateDiff_avg20 = mavg(turnoverRateDiff,20) context by symbol; // 日间收益率均值 
        update data set turnoverRateDiff_std20 = mstd(turnoverRateDiff,20) context by symbol; // 日间收益率波动率
        
        // 日内收益率
        update data set intraDayReturn = prev(nullFill((close-open)\open,0.0)) context by symbol;
        update data set intraDayReturn_avg20 = mavg(intraDayReturn,20) context by symbol; // 日内收益率均值 
        update data set intraDayReturn_std20 = mstd(intraDayReturn,20) context by symbol; // 日内收益率波动率
        
        // 日间反转-波动翻转因子
        update data set interDayVolatilityReverse = interDayReturn_avg20
        update data set interDayVolatilityReverse = -1.0 * interDayVolatilityReverse where interDayReturn_std20<avg(interDayReturn_std20) context by date;
        update data set interDayVolatilityReverse_avg20 = mavg(interDayVolatilityReverse,20) context by symbol
        update data set interDayVolatilityReverse_std20 = mstd(interDayVolatilityReverse,20) context by symbol
         
        // 日间反转-换手翻转因子
        update data set interDayTurnoverRateReverse = interDayReturn_avg20
        update data set interDayTurnoverRateReverse = -1.0 * interDayTurnoverRateReverse where turnoverRateDiff_std20<avg(turnoverRateDiff_std20) context by date;
        update data set interDayTurnoverRateReverse_avg20 = mavg(interDayTurnoverRateReverse,20) context by symbol
        update data set interDayTurnoverRateReverse_std20 = mstd(interDayTurnoverRateReverse,20) context by symbol
         
        // 日内反转-波动翻转因子
        update data set intraDayVolatilityReverse = intraDayReturn_avg20
        update data set intraDayVolatilityReverse = -1.0 * intraDayVolatilityReverse where intraDayReturn_std20<avg(intraDayReturn_std20) context by date;
        update data set intraDayVolatilityReverse_avg20 = mavg(intraDayVolatilityReverse,20) context by symbol
        update data set intraDayVolatilityReverse_std20 = mstd(intraDayVolatilityReverse,20) context by symbol
         
        // 日内反转-换手翻转因子
        update data set intraDayTurnoverRateReverse = intraDayReturn_avg20
        update data set intraDayTurnoverRateReverse = -1.0 * intraDayTurnoverRateReverse where turnoverRateDiff_std20<avg(turnoverRateDiff_std20) context by date;
        update data set intraDayTurnoverRateReverse_avg20 = mavg(intraDayTurnoverRateReverse,20) context by symbol
        update data set intraDayTurnoverRateReverse_std20 = mstd(intraDayTurnoverRateReverse,20) context by symbol
         
        // 截面空缺值填充
        pt = sql(select = [sqlCol(`symbol),sqlCol(`date),sqlCol(`minute),sqlCol(`circ_mv)].append!(sqlCol(factor_list)), from=data).eval()
        // 截面空缺值填充
    ...
    def addLongDayFreqFactorFromMinFreqData(self: ReturnModel_Backtest):
