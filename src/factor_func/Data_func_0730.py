...
class ReturnModel_Data:
    ...
    def prepare_factor_data_long_mr(self: ReturnModel_Backtest):
        ...
        self.session.run(f"""
          ...
          open,high,low,close,volume,turnover from loadTable("dfs://MinuteKDB","stock_bar") where TradeDate between temporalAdd(date({self.start_dot_date}),-1,"d") and date({self.end_dot_date})>,`SecurityID); // 这里根据因子情况修改-1为状态函数的最大时间跨度
           ...
           // 分钟频时序因子
          defg shioFunc(x){{ // defg 聚合函数声明
              idx_min = imin(x);
              idx_max = imax(x);
              res = (x[idx_max]-x[idx_min])\(x[idx_min])\(1+idx_max-idx_min)
              return res
          }}
          ...
          def FactorMap(data){{
              ...
              update pt set wVOL = msum(volume,9) context by symbol;
              factor_list = ["shio_60","shio_120","shio_180","shio_240","shio_480"];
              update pt set shio_60 = moving(shioFunc,wVOL,60,1) context by symbol;
              update pt set shio_120 = moving(shioFunc,wVOL,120,1) context by symbol;
              update pt set shio_180 = moving(shioFunc,wVOL,180,1) context by symbol;
              update pt set shio_240 = moving(shioFunc,wVOL,240,1) context by symbol;
              update pt set shio_480 = moving(shioFunc,wVOL,480,1) context by symbol;
    ...
    def prepare_factor_data_long(self: ReturnModel_Backtest):
        ...
        data = select SecurityID as symbol,TradeDate as date,int(string(TradeTime.hourOfDay())+lpad(string(TradeTime.minuteOfHour()),2,"0")) as minute,
        open,high,low,close,volume,turnover from loadTable("dfs://MinuteKDB","stock_bar") where (TradeDate between temporalAdd(date({self.start_dot_date}),-1,"d") and date({self.end_dot_date})) and SecurityID in code_list;
    ...
    def prepare_combine_data(self: ReturnModel_Backtest):
        ...
        // NBR240
        update symbol_df set NBR240 = (move(close,-240)-close)/close context by symbol;
        update symbol_df set NBR240 = NBR240.ffill() context by symbol;
