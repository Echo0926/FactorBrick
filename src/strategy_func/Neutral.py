...
from src.strategy_func.template import neutral_strategy
...
def get_stock_signal_to_dataframe(session, BackTest_config: Dict, ReturnModel_config: Dict):
    ...
    res_signal = session.run(rf"""
       ...
       init_period = 2; // 因子模型初始化period周期长度, 与returnModel保持一致
        ...
        // 行情数据
       k_data = select * from loadTable("dfs://stock_cn/combination","base") where date between start_ts and end_ts;

       // 预期收益率数据
       r_data = select date,minute,symbol,lightgbm_return_pred as return_pred from loadTable("dfs://asset_cn/ReturnModel_MLModel_20250425","ModelIndividual_return");
       ...
       update r_data set daily_long_signal = 1.0 where positive_rank <= percentile(positive_rank,10) context by date;  // 交易预期收益率前20%的票
       update r_data set daily_short_signal = 1.0 where negative_rank <= percentile(negative_rank,10) context by date;
       ...
       sortBy!(k_data,`timestamp`symbol,[1,1]);    // 这里一定要排序!
       ...
       // 这里指定为后五分钟的open的均值
       update k_data set next_price = 0.2 * (move(open,-1)+move(open,-2)+move(open,-3)+move(open,-4)+move(open,-5)) context by symbol+string(date);
       update k_data set buy_signal = next_price;
       update k_data set sell_signal = next_price;
       update k_data set buy_signal = 0.0 where minute != firstNot(minute) context by symbol+string(date);
       update k_data set buy_signal = buy_signal * daily_long_signal; 
       update k_data set sell_signal = 0.0 where minute != firstNot(minute) context by symbol+string(date);
...
def Neutral_strategy(self: Backtest):
    ...
    中性策略执行Pipeline
    """
    res_strategy = neutral_strategy.Neutral_strategy(self,
                                                 stock_pos_limit=0.2,  # 每次的杠杆水平
