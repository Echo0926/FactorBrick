...
class ReturnModel_Plot:
    def __init__(self,session,pool,config,
                 asset_return_Algo="nullFill((open-prev(open))/prev(open),0)",
        ...
        self.result_database=config["result_database"]    # 【新增:result_database】
        self.model_database=config["model_database"]      #  [新增：model_database]
        self.optimize_database=config["optimize_database"]

        # 变量类
        self.label_pred=config["label_pred"]
        ...
        # 单因子结果
        self.SingleFactor_estimation = config["SingleFactor_estimation"]
        ...
        # 多因子结果
        self.MultiFactor_estimation = config["MultiFactor_estimation"]
        ...
        self.Model_list=config["Model_list"]  # 自定义模型list(顺序不能乱)
        ...
        self.optstrategy_list=config["optstrategy_list"]  # 组合优化使用策略list
