...
class ReturnModel_Plot(ReturnModel_Backtest_mr):
    ...
    def FactorR_plot(self: ReturnModel_Backtest_mr):
        ...
        pt=select * from loadTable("{self.result_database}","{self.summary_table}") 
            where Benchmark="{benchmark}" and indicator ilike "%{factor}" 
            and (class ilike "%_{method}" or class in ["IC","RankIC"]);
        ...
        quantile_pt=select * from loadTable("{self.result_database}","{self.quantile_table}") where Benchmark == "{benchmark}" and factor == "{factor}"
        quantile_pt = lj(template_pt,quantile_pt,`period)
        quantile_pt = select * from quantile_pt where start_date>=date({self.start_dot_date}) and end_date<=date({self.end_dot_date});    
        undef(`template_pt);
        ...
        for (r_interval in returnIntervals){{  // 这里只统计累计值(cumsum)
            df = sql(select=[sqlColAlias(<start_date>,`date)].append!(sqlCol("QuantileReturn"+string(1..{self.Quantile}))),
                    from=quantile_pt, where=<ReturnInterval == r_interval>).eval()
            ts_list = df[`date];
            dropColumns!(df,`date);
            df = cumsum(df) + 1
            Dict["Return"+string(r_interval)] = select ts_list as `date, * from df
        }}
        undef(`pt); // 清除缓存
        ...
        with tabQuantile:
            for r_interval in self.returnIntervals:
                ...
                st.line_chart(data=Dict["Return" + str(r_interval)], x="date", y=None)
        with tabReg:
    ...
    def GroupReturn_Plot(self: ReturnModel_Backtest_mr):
        ...
        for tab, group in zip(tabs, self.Group_list):
            with tab:
                ...
                # 回测结果
                total_group_list = Dict[
                    "total_group_list"]  # return_(每日组合收益率)/Cumreturn_(累计组合收益率)/Bench_(每日基准收益率)/CumBench_(累计基准收益率)
                ...
                st.bar_chart(data=Return_df, x="date",
                             y=["Return_" + str(i) for i in select_groups] + ["Bench_" + str(i) for i in select_groups],
                             stack=False)
                st.subheader("PortFolio CumprodReturn", divider=True)
                st.line_chart(data=Cumprod_df, x="date",
                              y=["CumReturn_" + str(i) for i in select_groups] + ["CumBench_" + str(i) for i in
                                                                                  select_groups])
    ...
    def OptimizeReturnPlot(self: ReturnModel_Backtest_mr):
        ...
        """)
        daily_return, cumprod_return, bench_return, benchcumprod_return = Dict["daily_return"], Dict["cumprod_return"], \
                                                                          Dict["bench_return"], Dict[
                                                                              "benchcumprod_return"]
        ...
        st.bar_chart(data=Return_df, x="date",
                     y=["Return_" + str(i) for i in strategy_list] + ["Bench_" + str(i) for i in strategy_list],
                     stack=False)
        st.subheader("PortFolio CumprodReturn (Optimization)", divider=True)
        st.line_chart(data=Cumprod_df, x="date",
                      y=["CumReturn_" + str(i) for i in strategy_list] + ["CumBench_" + str(i) for i in strategy_list])
        return Dict


if __name__ == "__main__":
    with open(r"D:\DolphinDB\Project\FactorBrick\src\config\returnmodel_config0823.json5", mode="r",
